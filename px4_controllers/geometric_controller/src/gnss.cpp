...
bool targetCallback(controller_msgs::start::Request &req, controller_msgs::start::Response &res)
{
  ...
  if (!target_init)
  {
    gps_target.clear();
    seqID = req.seq;
    setpoint_type = req.type;

    switch (setpoint_type)
    {
    case SETPOINT_TYPE::SETPOINT_TYPE_GPS:
    ...
    case SETPOINT_TYPE::SETPOINT_TYPE_LOCAL:
      for (auto p : req.target.poses)
      {
        Eigen::Vector3d local_element;
        local_element << p.position.x, p.position.y, p.position.z;
        local_setpoint.push_back(local_element);
      }
      if (local_setpoint.size() > 0)
        target_init = true;
      break;
...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    {
      sample_idx++;
      if (sample_idx > Indexwp[waypoint_idx])
        waypoint_idx++;
