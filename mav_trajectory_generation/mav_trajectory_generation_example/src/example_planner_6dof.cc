...
ExamplePlanner::ExamplePlanner(ros::NodeHandle& nh)
  ...
  pub_point_ =
      nh.advertise<visualization_msgs::Marker>("point", 0);
  ...
  sub_odom_ =
      nh.subscribe("uav_odom", 1, &ExamplePlanner::uavOdomCallback, this);
}

// Callback to get current Pose of UAV
void ExamplePlanner::uavOdomCallback(const nav_msgs::Odometry::ConstPtr& odom) {
  ROS_INFO_STREAM("got_pos");
...
bool ExamplePlanner::planMultiTrajectory(
  ...
  if (dimension == 6) 
  {
    ...
    for (auto p : goal_pos) {
        ...
        Eigen::Matrix<double, -1, 1> rot(3, 1);
        rot << p.tail(3);
    ...
    for (auto v : goal_vel) {
        ...
        Eigen::Matrix<double, -1, 1> ang_vel(3, 1);
        ang_vel << v.tail(3);
        goal_ang_vel.push_back(ang_vel);
    }
    
    success = planMultiTrajectory(
      goal_position,goal_lin_vel,current_pose_.translation(),current_velocity_,max_v_,max_a_,&trajectory_trans);
  ...
  { 
    ...
    success = planMultiTrajectory(
        goal_position, goal_lin_vel, current_pose_.translation(), current_velocity_,
        max_v_, max_a_, &(*trajectory));
    return success;
  } 
  else if (dimension == 4) 
  { 
    // std::vector <Eigen::Vector4d> goal_position,goal_lin_vel,goal_rotation,goal_ang_vel;
    // for(auto p : goal_pos){
    //   goal_position.push_back(p.head(4));
    // }
    // for(auto v : goal_vel){
    //   goal_lin_vel.push_back(v.head(4));
    // }
    ...
    // success = planMultiTrajectory(
    //     goal_position, goal_lin_vel, start_pos_4d, start_vel_4d, max_v_, max_a_,
    //     &(*trajectory));
    return false;
  } 
  else 
...
bool ExamplePlanner::planMultiTrajectory(const std::vector<Eigen::VectorXd>& goal_pos,
...
bool ExamplePlanner::publishPoint(const Eigen::VectorXd& point){
  // send trajectory as markers to display them in RVIZ
  ...
  markers.scale.y = 0.4;
  markers.scale.z = 0.4; 
  markers.pose.position.x = point[0];
  markers.pose.position.y = point[1];
  markers.pose.position.z = point[2];
  markers.color.a = 1;
  markers.color.r = 1;
